...
This is a heavily modified version of the Android Open Source Project (AOSP) fastboot daemon (`fastbootd`), customized for Raspberry Pi secure boot and device provisioning workflows. While maintaining compatibility with the standard fastboot protocol, it adds extensive Raspberry Pi-specific functionality for partition management, encryption, dm-verity, GPIO control, and Interactive Device Provisioning (IDP).
...
- **Interactive Device Provisioning (IDP)** - JSON-driven partition layout and provisioning
...
**Interactive Device Provisioning (IDP) example:**
...
### Interactive Device Provisioning (IDP)
