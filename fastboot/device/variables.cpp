...
bool GetMmcExtCsd(FastbootDevice* device) {
    ...
    // sysfs exposes the raw CSD register; spec_vers is bits [125:122] (byte0[5:2]).
    std::string csd_str;
    if (!android::base::ReadFileToString("/sys/block/mmcblk0/device/csd", &csd_str)) {
        return device->WriteFail("Cannot read CSD register (no /sys/block/mmcblk0/device/csd)");
    }
    csd_str.erase(csd_str.find_last_not_of(" \t\n\r") + 1);
    if (csd_str.size() < 2) {
        return device->WriteFail("CSD register too short: \"" + csd_str + "\"");
    }
    unsigned int byte0 = 0;
    if (sscanf(csd_str.c_str(), "%2x", &byte0) != 1) {
        return device->WriteFail("Cannot parse CSD register: \"" + csd_str + "\"");
    }
    // CSD[127:126] = csd_structure, CSD[125:122] = spec_vers
    // byte0 = CSD[127:120], so spec_vers = (byte0 >> 2) & 0xF
