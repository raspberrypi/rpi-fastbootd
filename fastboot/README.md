...
This project uses `dpkg-buildpackage` (via `./build-package.sh`) to generate `.deb` packages.

### Creating Packages

```bash
# Standard build (GPLv3)
./build-package.sh
...
| **TCP/IP** | Development, debugging, network provisioning | `fastboot -s tcp:IP devices` |
